...
BASE_DIR=/global/scratch/projects/fc_wolflab/bbioinfo/empchase/synthetic_contaminant/A8thetaVPI-90_A4thetaWH502-10
...
READ1_PATH=/global/scratch/projects/fc_wolflab/bbioinfo/empchase/synthetic_contaminant/A8thetaVPI-90_A4thetaWH502-10/sampleA8+A4_read1.fastq.gz
READ2_PATH=/global/scratch/projects/fc_wolflab/bbioinfo/empchase/synthetic_contaminant/A8thetaVPI-90_A4thetaWH502-10/sampleA8+A4_read2.fastq.gz
# SAMPLE_NAME=$(echo "$LINE" | awk -F'\t' '{print $3}')
SAMPLE_NAME="sampleA8+A4_90_10"
# PLATE=$(echo "$LINE" | awk -F'\t' '{print $4}')
# LIBRARY_NAME=$(echo "$LINE" | awk -F'\t' '{print $5}')
ADAPTER_FILE=/global/scratch/projects/fc_wolflab/bbioinfo/empchase/synthetic_contaminant/A8thetaVPI-90_A4thetaWH502-10/A8+A4_adapters.fasta
...
# fastqc -o "${FASTQC_OUTPUT_DIR}" "${OUTPUT_DIR}/${SAMPLE_NAME}_output_forward_paired.fq.gz"
...
spades.py  -m 46 -o . --pe1-1 "${OUTPUT_DIR}/${SAMPLE_NAME}_output_forward_paired.fq.gz" --pe1-2 "${OUTPUT_DIR}/${SAMPLE_NAME}_output_reverse_paired.fq.gz"


echo "Assembly completed for sample ${SAMPLE_NAME}, Library ${LIBRARY_NAME}"
